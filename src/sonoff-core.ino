...
void Sonoff::toggle() {
  ...
  if (Configuration.debugger)
    Serial << endl << "WARN: Rebooting device";
  Led.blink();
  delay(10);
  ESP.restart();
}
...
void Sonoff::connectWiFi() {
  ...
  if (Configuration.debugger)
    Serial << endl << "INFO: Connecting to WiFi: " << Configuration.wifi_ssid;
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    if (Configuration.debugger)
      Serial << endl
             << "INFO: WiFi connection attempt: " << connection_try << " from "
             << Configuration.number_connection_attempts;
    ...
    if (Configuration.mode == MODE_SWITCH) {
      if (Configuration.interface == INTERFACE_MQTT) {
        if (WiFi.status() == WL_CONNECTED) {
          ...
          if (Configuration.dht_present) {
            if (millis() >=
                temperatureTimer + (Configuration.dht_interval * 1000)) {
              SonoffDHT DHTSesnor;
              float temperature = DHTSesnor.getTemperature();
              publishTemperature(temperature);
              temperatureTimer = humidityTimer + 9999; // @TODO do it better
            }
            if (millis() >=
                humidityTimer + ((Configuration.dht_interval + 2) * 1000)) {
              SonoffDHT DHTSesnor;
              float humidity = DHTSesnor.getHumidity();
              temperatureTimer = millis();
              humidityTimer = temperatureTimer;
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          if (Configuration.domoticz_publish_relay_state) {
            DomoticzInterface.pushRequest(); // If calls URL to Domotoczu with
                                             // eg status of switch
          }
        } else {
          runSleepMode(); // Not connected to WiFi
      ...
      if (Configuration.debugger)
        Serial << endl << "ERROR: Internal Application Error";
    }
  }
}

void Sonoff::setDHTInterval(unsigned int interval) {
  // temperatureTimer.detach();
  //  temperatureTimer.attach(interval, callbackDHT);
}
...
void Sonoff::publishHumidity(float humidity) {
  if (previousHumidity != humidity) {
    char humidityString[6];
    dtostrf(humidity, 2, 2, humidityString);
    MqttInterface.publish((char *)"humidity", humidityString);
...
void Sonoff::stopSleepMode() {
  ...
  if (Configuration.debugger)
    Serial << endl << "INFO: Sleep mode turned off";
}
...
void Sonoff::runSleepMode() {
  if (Configuration.debugger)
    Serial << endl << "WARN: Going to sleep mode";
  Configuration.sleep_mode = true;
  Led.startBlinking(1);
  sleepModeTimer.attach(
      Configuration.duration_between_next_connection_attempts_series * 60,
      callbackSleepMode);
}
...
void Sonoff::runSwitch() {
  Led.on();
  if (Configuration.debugger)
    Serial << endl << "INFO: Device mode: SWITCH";
  if (Configuration.interface !=
      INTERFACE_NONE) {  /* If not standalone mode connect to WiFi and run
                            DS18B20 sensor if configured */
  ...
  if (Configuration.switch_present) { /* Run external switch if configured */
    if (Configuration.debugger)
      Serial << endl << "INFO: Initiating external switch";
    Switch.init(Configuration.switch_gpio, Configuration.switch_type);
  } else {
    if (Configuration.debugger)
      Serial << endl << "INFO: External switch not present";
  }
...
void Sonoff::runConfigurationLAN() {
  Led.on();
  if (Configuration.debugger)
    Serial << endl << "INFO: Device mode: LAN Configuration";
  WiFi.mode(WIFI_STA);
  connectWiFi();
  if (!Configuration.sleep_mode) {
    startHttpServer();
    ...
    if (Configuration.debugger)
      Serial << endl
             << "WARN: Not connected to WiFi - configuration is not possible. "
                "Try Access Point mode - press button for 12s";
    toggle();
  }
  WiFi.mode(WIFI_STA);
}
...
void Sonoff::setRelayAfterConnectingToMQTT() {
  if (Configuration.debugger)
    Serial << endl << "INFO: Setting relay post MQTT connection";
  if (Eeprom.getRelayStateAfterConnectionRestored() == DEFAULT_RELAY_ON) {
    Relay.on();
  } else if (Eeprom.getRelayStateAfterConnectionRestored() ==
             DEFAULT_RELAY_OFF) {
    Relay.off();
  } else if (Eeprom.getRelayStateAfterConnectionRestored() ==
             DEFAULT_RELAY_LAST_KNOWN) {
...
boolean Sonoff::isConfigured() {
  if (Configuration.wifi_ssid[0] == (char)0 ||
      Configuration.wifi_password[0] == (char)0) {
    if (Configuration.debugger)
      Serial << endl
             << "WARN: Missing configuration. Going to configuration mode.";
...
void callbackSleepMode() {
  if (Configuration.debugger)
    Serial << endl << "INFO: Checking if connected to WiFi.";
  if (WiFi.status() == WL_CONNECTED) {
